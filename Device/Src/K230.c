...
{
    uint8_t Snapshot[K230_RX_BUFFER_SIZE];
    ...
    __disable_irq();
    if(!K230_Buffer.BufferFlag){
        if(!Primask) __enable_irq();
        return 0;
    }

    Size = K230_Buffer.Size;
    if(Size > K230_RX_BUFFER_SIZE) Size = K230_RX_BUFFER_SIZE;
    for(Index = 0; Index < (int32_t)Size; Index++){
        Snapshot[Index] = K230_Buffer.Readptr[Index];
    }
    K230_Buffer.BufferFlag = 0;
    if(!Primask) __enable_irq();
